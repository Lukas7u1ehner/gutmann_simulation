...
try:
    from src.style import GUTMANN_ACCENT_GREEN, GUTMANN_LIGHT_TEXT, GUTMANN_SECONDARY_DARK, GUTMANN_DARK_GREEN
except ImportError:
...
def create_simulation_chart(df_history: pd.DataFrame, df_forecast: pd.DataFrame = None):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=GUTMANN_LIGHT_TEXT, width=2, dash='dash') # Heller Text, gestrichelt
    ))
    ...
    if df_forecast is not None and not df_forecast.empty:
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=PROGNOSE_NOMINAL_COLOR, width=2.5, dash='dash') # (NEUE FARBE)
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=PROGNOSE_REAL_COLOR, width=2, dash='dot') # (NEUE FARBE)
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=PROGNOSE_EINZAHLUNG_COLOR, width=2.5, dash='dash') # (NEUE FARBE)
        ))
